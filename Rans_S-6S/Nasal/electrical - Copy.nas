...
############################################################################
############# Définition des proppriété à  l'initialisation ################
############################################################################
...
var update_virtual_bus = func(dt) {
    ...
    #####################################################################
    ################# DÃ©finition de la tension au moteur ################
    #####################################################################

    if(PWR){										# Si le systéme électrique n'est pas endommagé
      ...
      elsif (ALT_L.getBoolValue() and (AltAmps_L < BatAmps)){				# Sinon si le switch Alt Left ou Alt Right est à  ON mais que le courant de l'alternateur est inf&rieur à ce lui de la batterie
        if (BATT.getBoolValue()){							# Si le switch Batt est Ã  ON 
    ...
    } else {										# Sinon c'est le systéme électrique est endommagé
        bus_volts = 0.0;								# La tension du Bus est 0V
    }

    props.globals.getNode("/engines/engine[0]/amp-v",1).setValue(bus_volts);		# La tension au moteur est égale Ã  celle du Bus
    props.globals.getNode("/engines/engine[1]/amp-v",1).setValue(bus_volts);		# La tension au moteur est égale Ã  celle du Bus
